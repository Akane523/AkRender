...
{
  Manifest manifest;
  open(manifest) | module("math").sources({"math.slang"}).import_as("math_utils") |
      build();
...
TEST(BuilderTest, UsesModuleRefInImperativeCommit)
{
  ...
  const ModuleRef math =
      module(reg, "math").sources({"math.slang"}).import_as("math_utils").commit();
...
{
  Manifest manifest;
  open(manifest) | spirv_file("legacy")
...
TEST(BuilderTest, SpirvShaderRefFromImperativeCommit)
{
  ...
  const SpirVShaderRef prebuilt =
      spirv_file(reg, "prebuilt")
          .source("frag.spv")
          .entry("main")
          .stage(Stage::Fragment)
          .at({"/spv/frag.spv"})
...
TEST(BuilderTest, UsesMultipleModuleRefsInImperativeCommit)
{
  ...
  ManifestRegister reg = open(manifest);
  const ModuleRef math =
      module(reg, "math").sources({"math.slang"}).commit();
...
{
  Manifest manifest;
  EXPECT_THROW(open(manifest) | module("empty") | build(), std::invalid_argument);
}

TEST(BuilderTest, SlangRejectsMissingSource)
{
  Manifest manifest;
  EXPECT_THROW(open(manifest) | slang("vert").entry("vsMain").stage(Stage::Vertex).ir() |
                   build(),
               std::invalid_argument);
}
...
TEST(BuilderTest, SlangRejectsMissingMode)
{
  Manifest manifest;
  EXPECT_THROW(
      open(manifest) | slang("vert").source("vert.slang").entry("vsMain").stage(Stage::Vertex) |
          build(),
      std::invalid_argument);
}
...
{
  Manifest manifest;
  EXPECT_THROW(open(manifest) | slang("vert").source("vert.slang").entry("vsMain").ir() |
                   build(),
               std::invalid_argument);
}
...
{
  Manifest manifest;
  EXPECT_THROW(open(manifest) | spirv_file("prebuilt").source("frag.spv").entry("main") |
                   build(),
               std::invalid_argument);
}
